// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'dashboard_providers.dart';

// **************************************************************************
// RiverpodGenerator
// **************************************************************************

String _$dashboardRemoteDataSourceHash() =>
    r'2458a6984999fb62c8bf80b5c931a23b2ac3c590';

/// See also [dashboardRemoteDataSource].
@ProviderFor(dashboardRemoteDataSource)
final dashboardRemoteDataSourceProvider =
    Provider<DashboardRemoteDataSource>.internal(
      dashboardRemoteDataSource,
      name: r'dashboardRemoteDataSourceProvider',
      debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
          ? null
          : _$dashboardRemoteDataSourceHash,
      dependencies: null,
      allTransitiveDependencies: null,
    );

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef DashboardRemoteDataSourceRef = ProviderRef<DashboardRemoteDataSource>;
String _$dashboardLocalDataSourceHash() =>
    r'5f2bfaa4d476a127ae5e14a05b29a80a166d422f';

/// See also [dashboardLocalDataSource].
@ProviderFor(dashboardLocalDataSource)
final dashboardLocalDataSourceProvider =
    Provider<DashboardLocalDataSource>.internal(
      dashboardLocalDataSource,
      name: r'dashboardLocalDataSourceProvider',
      debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
          ? null
          : _$dashboardLocalDataSourceHash,
      dependencies: null,
      allTransitiveDependencies: null,
    );

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef DashboardLocalDataSourceRef = ProviderRef<DashboardLocalDataSource>;
String _$dashboardRepositoryHash() =>
    r'736465f2e6ed703c19cce8317f183fbaa4aeb2f2';

/// See also [dashboardRepository].
@ProviderFor(dashboardRepository)
final dashboardRepositoryProvider = Provider<DashboardRepository>.internal(
  dashboardRepository,
  name: r'dashboardRepositoryProvider',
  debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
      ? null
      : _$dashboardRepositoryHash,
  dependencies: null,
  allTransitiveDependencies: null,
);

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef DashboardRepositoryRef = ProviderRef<DashboardRepository>;
String _$getDashboardDataUseCaseHash() =>
    r'743ce171ad80e86487f7644cff57cba7eb2252a7';

/// See also [getDashboardDataUseCase].
@ProviderFor(getDashboardDataUseCase)
final getDashboardDataUseCaseProvider =
    Provider<GetDashboardDataUseCase>.internal(
      getDashboardDataUseCase,
      name: r'getDashboardDataUseCaseProvider',
      debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
          ? null
          : _$getDashboardDataUseCaseHash,
      dependencies: null,
      allTransitiveDependencies: null,
    );

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef GetDashboardDataUseCaseRef = ProviderRef<GetDashboardDataUseCase>;
String _$getEducationItemsUseCaseHash() =>
    r'efdec5afde6e3ecb04155ccd8add549fb265c138';

/// See also [getEducationItemsUseCase].
@ProviderFor(getEducationItemsUseCase)
final getEducationItemsUseCaseProvider =
    Provider<GetEducationItemsUseCase>.internal(
      getEducationItemsUseCase,
      name: r'getEducationItemsUseCaseProvider',
      debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
          ? null
          : _$getEducationItemsUseCaseHash,
      dependencies: null,
      allTransitiveDependencies: null,
    );

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef GetEducationItemsUseCaseRef = ProviderRef<GetEducationItemsUseCase>;
String _$markEducationAsReadUseCaseHash() =>
    r'a740f705e676905ffc354c10bb3dd41e6eac4220';

/// See also [markEducationAsReadUseCase].
@ProviderFor(markEducationAsReadUseCase)
final markEducationAsReadUseCaseProvider =
    Provider<MarkEducationAsReadUseCase>.internal(
      markEducationAsReadUseCase,
      name: r'markEducationAsReadUseCaseProvider',
      debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
          ? null
          : _$markEducationAsReadUseCaseHash,
      dependencies: null,
      allTransitiveDependencies: null,
    );

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef MarkEducationAsReadUseCaseRef = ProviderRef<MarkEducationAsReadUseCase>;
String _$toggleEducationFavoriteUseCaseHash() =>
    r'1213e0b5f0d2f7d505895b01c15fed16478b904a';

/// See also [toggleEducationFavoriteUseCase].
@ProviderFor(toggleEducationFavoriteUseCase)
final toggleEducationFavoriteUseCaseProvider =
    Provider<ToggleEducationFavoriteUseCase>.internal(
      toggleEducationFavoriteUseCase,
      name: r'toggleEducationFavoriteUseCaseProvider',
      debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
          ? null
          : _$toggleEducationFavoriteUseCaseHash,
      dependencies: null,
      allTransitiveDependencies: null,
    );

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef ToggleEducationFavoriteUseCaseRef =
    ProviderRef<ToggleEducationFavoriteUseCase>;
String _$calculateHealthScoreUseCaseHash() =>
    r'7bc8a6bcaaf92cf727cde4e7f68882f9b4c41cfe';

/// See also [calculateHealthScoreUseCase].
@ProviderFor(calculateHealthScoreUseCase)
final calculateHealthScoreUseCaseProvider =
    Provider<CalculateHealthScoreUseCase>.internal(
      calculateHealthScoreUseCase,
      name: r'calculateHealthScoreUseCaseProvider',
      debugGetCreateSourceHash: const bool.fromEnvironment('dart.vm.product')
          ? null
          : _$calculateHealthScoreUseCaseHash,
      dependencies: null,
      allTransitiveDependencies: null,
    );

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef CalculateHealthScoreUseCaseRef =
    ProviderRef<CalculateHealthScoreUseCase>;
// ignore_for_file: type=lint
// ignore_for_file: subtype_of_sealed_class, invalid_use_of_internal_member, invalid_use_of_visible_for_testing_member, deprecated_member_use_from_same_package
